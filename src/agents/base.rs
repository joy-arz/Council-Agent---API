...
impl base_agent {
    ...
    pub fn build_full_system_prompt(&self) -> String {
        let safety_instruction = if self.is_autonomous {
            "you are in autonomous mode. you are an active engineering partner. you have full permission and are EXPECTED to use your internal tools (like write_file, replace, run_shell_command) to directly modify the codebase. your goal is to drive the workflow forward until the task is complete. you have full access to the workspace root and all its subfolders recursively. do not just talk; ACT by reading and editing any necessary files in the workspace (including nested folders). you do not need to ask for permission for each step; assume ALL changes are pre-accepted and proceed immediately with the implementation."
        } else {
